...
const CompressPdf = () => {
  ...
  const handleCompress = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error((await res.json()).error || 'Failed to compress PDF');
  ...
  return (
    <div className="container" style={{ padding: '4rem 0', maxWidth: '800px' }}>
      <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
        <h1 className="text-gradient" style={{ fontSize: '3rem', marginBottom: '1rem' }}>Compress PDF</h1>
        <p style={{ color: 'var(--text-secondary)', fontSize: '1.2rem' }}>Reduce your PDF file size while keeping the content intact.</p>
      </div>
      ...
      <div className="glass-card">
        {!file ? (
          ...
          <div style={{ padding: '2rem', textAlign: 'center' }}>
            ...
            <p style={{ color: 'var(--text-secondary)', marginBottom: '2rem' }}>{(file.size / 1024 / 1024).toFixed(2)} MB</p>
            {done && <p style={{ color: '#22c55e', marginBottom: '1rem' }}>✅ File compressed and downloaded!</p>}
            <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center' }}>
              <button className="btn btn-secondary" onClick={() => { setFile(null); setDone(false); }} disabled={isLoading}>Cancel</button>
              <button className="btn btn-primary" onClick={handleCompress} disabled={isLoading}>
                {isLoading ? <><Loader2 size={18} /> Compressing...</> : <><Download size={18} /> Compress & Download</>}
              </button>
            </div>
          </div>
        )}
